...
```{r, eval = FALSE}
rock_garden <- download_transect(transect_id = 6875)
kaintuck <- download_transect_list(database_id = 63,
                                   site == "kaintuck")
```
...
Each floristic quality assessments on [www.universalFQA.org](www.universalFQA.org) includes two sorts of information: details about the species observed during data collection and summary information about the assessment as a whole. The ${\tt fqar}$ functions ${\tt assessment\_inventory()}$ and ${\tt assessment\_glance()}$, respectively, extract and tidy this information.
